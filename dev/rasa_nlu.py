...
wordsToRemove = ['corona', 'coronavirus',
                 'covid', 'covid-19', 'virus', 'illness']
...
with open("markdown.md", "w+") as f:
    ...
    for key in jsonData.keys():
        ...
        for i in range(3):
            print(
                f"- {re.sub('[^A-Za-z0-9]+ ', '', ' '.join(text[text.index(longest_words[i])-1:text.index(longest_words[i])+2]))}", file=f)

        for i in range(3):
            print(
                f"- {re.sub('[^A-Za-z0-9]+ ', '', longest_words[i])}", file=f)
